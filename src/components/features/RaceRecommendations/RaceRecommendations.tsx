...
export function RaceRecommendations({
  race,
  weatherData,
}: RaceRecommendationsProps) {
  const getRecommendedEquipment = (): Equipment[] => {
    ...
    if (race.terrain === "TRAIL") {
      ...
      equipment.push({
        ...
        features: ["缓震性能出色", "轻量化设计", "透气舒适"],
        alternatives: ["Nike Vaporfly", "Hoka Bondi", "Saucony Endorphin"],
      });
    }
    ...
    if (race.distance > 21) {
      equipment.push({
        ...
        features: ["GPS定位", "心率监测", "电池续航长"],
        alternatives: ["Garmin Forerunner", "Coros Apex", "Polar Vantage"],
      });
    }
    ...
    return [
      {
        ...
        tips: ["循序渐进增加训练量", "注意核心力量训练", "保持充足休息"],
      },
      {
        phase: "强化期 (4-8周)",
        duration: "4周",
        ...
        tips: ["模拟赛道环境训练", "注意补给策略练习", "调整装备适应性"],
      },
      {
        phase: "赛前调整 (1-2周)",
        duration: "2周",
